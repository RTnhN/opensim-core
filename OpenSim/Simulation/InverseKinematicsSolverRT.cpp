/* -------------------------------------------------------------------------- *
 *                   OpenSim:  InverseKinematicsSolverRT.cpp                    *
 * -------------------------------------------------------------------------- *
...
#include "InverseKinematicsSolverRT.h"
...
/*
 * An implementation of the InverseKinematicsSolverRT 
 *
 * @param model to assemble
 */
InverseKinematicsSolverRT::InverseKinematicsSolverRT(const Model& model,
        std::shared_ptr<MarkersReference> markersReference,
        SimTK::Array_<CoordinateReference>& coordinateReferences,
        double constraintWeight) :
        InverseKinematicsSolverRT(model, markersReference, nullptr,
...
InverseKinematicsSolverRT::InverseKinematicsSolverRT(const Model& model,
    ...
    if (_markersReference && _markersReference->getNumRefs() > 0) {
        ...
        if (modelMarkerSet.getSize() < 1) {
            log_error("InverseKinematicsSolverRT: Model has no markers!");
            throw Exception("InverseKinematicsSolverRT: Model has no markers!");
        }
        const SimTK::Array_<std::string>& markerNames
            = _markersReference->getNames(); // size and content as in trc file

        if (markerNames.size() < 1) {
            log_error("InverseKinematicsSolverRT: No markers available from data provided.");
            throw Exception("InverseKinematicsSolverRT: No markers available from data provided.");
        }
        int index = 0, cnt = 0;
        ...
        if (cnt < 1) {
            log_error("InverseKinematicsSolverRT: Marker data does not correspond to any model markers.");
            throw Exception("InverseKinematicsSolverRT: Marker data does not correspond to any model markers.");
        }
        if (cnt < 4)
            log_warn("WARNING: InverseKinematicsSolverRT found only {} markers to track.", cnt);
    ...
    if (_orientationsReference && _orientationsReference->getNumRefs() > 0) {
        ...
        if (sensorNames.size() < 1) {
            log_error("InverseKinematicsSolverRT: No orientation data is available from provided "
                      "source.");
            throw Exception("InverseKinematicsSolverRT: No orientation data is available "
        ...
        if (cnt < 1) {
            log_error("InverseKinematicsSolverRT: Orientation data does not "
                      "correspond to any model frames.");
            throw Exception("InverseKinematicsSolverRT: Orientation data does not "
                            "correspond to any model frames.");
        }
        log_info("InverseKinematicsSolverRT found {} model frames to track.",
                    cnt);

    }
}

int InverseKinematicsSolverRT::getNumMarkersInUse() const
{
...
void InverseKinematicsSolverRT::updateMarkerWeight(const std::string& markerName, double value)
{
...
void InverseKinematicsSolverRT::updateMarkerWeight(int markerIndex, double value)
{
    if(markerIndex >=0 && markerIndex < _markersReference->getMarkerWeightSet().getSize()){
        ...
        throw Exception("InverseKinematicsSolverRT::updateMarkerWeight: invalid markerIndex.");
}

/* Update all markers weights by order in the markersReference passed in to
   construct the solver. */
void InverseKinematicsSolverRT::updateMarkerWeights(const SimTK::Array_<double> &weights)
{
    if(static_cast<unsigned>(_markersReference->getMarkerWeightSet().getSize()) 
       == weights.size()){
        ...
        throw Exception("InverseKinematicsSolverRT::updateMarkerWeights: invalid size of weights.");
}

int InverseKinematicsSolverRT::getNumOrientationSensorsInUse() const
{
...
void InverseKinematicsSolverRT::updateOrientationWeight(const std::string& orientationName, double value)
{
...
void InverseKinematicsSolverRT::updateOrientationWeight(int orientationIndex, double value)
{
    if (orientationIndex >= 0 && orientationIndex < _orientationsReference->updOrientationWeightSet().getSize()) {
        ...
        throw Exception("InverseKinematicsSolverRT::updateOrientationWeight: invalid orientationIndex.");
}

/* Update all orientation sensor weights by order in the orientationsReference passed in to
construct the solver. */
void InverseKinematicsSolverRT::updateOrientationWeights(const SimTK::Array_<double> &weights)
{
    if (static_cast<unsigned>(_orientationsReference->updOrientationWeightSet().getSize())
        ...
        throw Exception("InverseKinematicsSolverRT::updateOrientationWeights: invalid size of weights.");
}

/* Compute and return the spatial location of a marker in ground. */
SimTK::Vec3 InverseKinematicsSolverRT::computeCurrentMarkerLocation(const std::string &markerName)
{
    const Array_<std::string> &names = _markersReference->getNames();
    SimTK::Array_<const std::string>::iterator p = std::find(names.begin(), names.end(), markerName);
    int index = (int)std::distance(names.begin(), p);
...
SimTK::Vec3 InverseKinematicsSolverRT::computeCurrentMarkerLocation(int markerIndex)
{
    if(markerIndex >=0 && markerIndex < _markerAssemblyCondition->getNumMarkers()){
        ...
        throw Exception("InverseKinematicsSolverRT::computeCurrentMarkerLocation: invalid markerIndex.");
}

/* Compute and return the spatial locations of all markers in ground. */
void InverseKinematicsSolverRT::computeCurrentMarkerLocations(SimTK::Array_<SimTK::Vec3> &markerLocations)
{
...
double InverseKinematicsSolverRT::computeCurrentMarkerError(const std::string &markerName)
{
    const Array_<std::string>& names = _markersReference->getNames();
    SimTK::Array_<const std::string>::iterator p = std::find(names.begin(), names.end(), markerName);
    int index = (int)std::distance(names.begin(), p);
...
double InverseKinematicsSolverRT::computeCurrentMarkerError(int markerIndex)
{
    if(markerIndex >=0 && markerIndex < _markerAssemblyCondition->getNumMarkers()){
        return _markerAssemblyCondition->findCurrentMarkerError(SimTK::Markers::MarkerIx(markerIndex));
    }
    else
        throw Exception("InverseKinematicsSolverRT::computeCurrentMarkerError: invalid markerIndex.");
}

/* Compute and return the distance errors between all model markers and their observations. */
void InverseKinematicsSolverRT::computeCurrentMarkerErrors(SimTK::Array_<double> &markerErrors)
{
...
double InverseKinematicsSolverRT::computeCurrentSquaredMarkerError(const std::string &markerName)
{
    const Array_<std::string>& names = _markersReference->getNames();
    SimTK::Array_<const std::string>::iterator p = std::find(names.begin(), names.end(), markerName);
    int index = (int)std::distance(names.begin(), p);
...
double InverseKinematicsSolverRT::computeCurrentSquaredMarkerError(int markerIndex)
{
    if(markerIndex >=0 && markerIndex < _markerAssemblyCondition->getNumMarkers()){
        return _markerAssemblyCondition->findCurrentMarkerErrorSquared(SimTK::Markers::MarkerIx(markerIndex));
    }
    else
        throw Exception("InverseKinematicsSolverRT::computeCurrentMarkerSquaredError: invalid markerIndex.");
}

/* Compute and return the distance errors between all model marker and observations. */
void InverseKinematicsSolverRT::
...
std::string InverseKinematicsSolverRT::getMarkerNameForIndex(int markerIndex) const
{
...
SimTK::Rotation InverseKinematicsSolverRT::
...
SimTK::Rotation InverseKinematicsSolverRT::computeCurrentSensorOrientation(int osensorIndex)
{
    if (osensorIndex >= 0 && osensorIndex < _orientationAssemblyCondition->getNumOSensors()) {
        ...
        throw Exception("InverseKinematicsSolverRT::computeCurrentOSensorOrientation: invalid osensorIndex.");
}

/* Compute and return the spatial locations of all markers in ground. */
void InverseKinematicsSolverRT::computeCurrentSensorOrientations(
...
double InverseKinematicsSolverRT::
...
double InverseKinematicsSolverRT::computeCurrentOrientationError(int osensorIndex)
{
    if (osensorIndex >= 0 && 
        ...
        throw Exception(
            "InverseKinematicsSolverRT::computeCurrentOrientationError: "
            "invalid sensor Index." );
}

/* Compute and return the distance between all model o-sensors and their observations. */
void InverseKinematicsSolverRT::computeCurrentOrientationErrors(
...
std::string InverseKinematicsSolverRT::
...
void InverseKinematicsSolverRT::setupGoals(SimTK::State &s)
{
...
void InverseKinematicsSolverRT::setupMarkersGoal(SimTK::State &s)
{
...
void InverseKinematicsSolverRT::setupOrientationsGoal(SimTK::State &s)
{
...
/* Internal method to update the time, reference values and/or their weights based
    on the state */
void InverseKinematicsSolverRT::updateGoals(SimTK::State &s)
{
    ...
    if (_orientationsReference && _orientationsReference->getNumRefs() > 0) {
        SimTK::Array_<SimTK::Rotation> orientationValues;
        log_info("Before get values at time");
        _orientationsReference->getValuesAtTime(nextTime, orientationValues);
        log_info("After get values at time");
        _orientationAssemblyCondition->moveAllObservations(orientationValues);
    }
    
}

// Implementation of updateOrientationData method
void InverseKinematicsSolverRT::updateOrientationData(double time, const SimTK::RowVectorView_<SimTK::Rotation_<double>>& dataRow)
{
